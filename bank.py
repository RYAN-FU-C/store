...
def deposit():
    ...
    if s is True:
        print("存款成功，现有金额：", bank[account]["money"])
    if s is False:
        print("账号不存在")
...
def withdraw():
    ...
    if s == 0:
        print("取款成功，剩余：", bank[account]["money"])
    elif s == 1:
        print("账号不存在")
    elif s == 2:
        print("密码不正确")
...
def bank_inquire(account):
    ...
    if account in keys:
        ...
        if password == bank[account]["password"]:
            ...
            print("用户居住地址：", bank[account]["country"], bank[account]["province"],
                  bank[account]["street"], bank[account]["door"])
            print("当前账户的开户行：", bank[account]["bank_name"])
        else:
            print("密码不正确")
    else:
        print("账户不存在")
